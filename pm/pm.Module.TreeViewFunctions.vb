...
#Region "Public"
    ...
    Public Shared Sub SaveTreeViewData(ByVal treeView As TreeView, ByVal path As String)
        ...
        file = Nothing
    End Sub
    Public Shared Function TreeViewToString(ByVal treeView As TreeView) As String
        Dim ser As New System.Xml.Serialization.XmlSerializer(GetType(TreeViewData))
        Dim mem As New System.IO.MemoryStream
        Dim writer As New System.Xml.XmlTextWriter(mem, Nothing)

        ser.Serialize(writer, New TreeViewData(treeView))


        mem.Position = 0
        Dim reader As New System.IO.StreamReader(mem)
    ...
    Public Shared Sub LoadTreeViewDataFromString(ByVal treeView As TreeView, ByVal xml As String)
        'Create as serializer and get the file to deserialize
        Dim ser As New System.Xml.Serialization.XmlSerializer(GetType(TreeViewData))
        Dim mem As New System.IO.MemoryStream
        ' Load the string into the memorystrema
        Dim bytes As Byte() = System.Text.Encoding.ASCII.GetBytes(xml)
        mem.Write(bytes, 0, bytes.Length)
        mem.Position = 0

        Dim reader As New System.Xml.XmlTextReader(mem)
